...
fn deterministic_limit_carries_each_cashflow_and_matches_standard_engine() {
    ...
    let expected_forward = (99.0 * growth(0.5) * 0.97 - 4.0) * growth(1.0) / growth(0.5) - 2.0;
    ...
    let actual = Plan::compile_bs(&req, rates(0.0), 0.0, 0.25, policy(1))
        ...
        .evaluate()
        .unwrap();
    let standard = SimulationPlan::compile(&req, policy(1))
        .unwrap()
        .execute()
        .unwrap();
...
fn deterministic_rate_limit_with_equity_vol_matches_shifted_black_reference() {
    ...
    let actual = Plan::compile_bs(&req, rates(0.0), 0.35, 0.25, policy(1))
        ...
        .unwrap();
    assert!(
        (actual.value - expected).abs() < 6.0 * actual.standard_error + 0.001,
        "{} != {expected}; se={}",
        actual.value,
...
fn escrowed_bs_aad_matches_spot_vol_and_every_curve_pillar_with_payment_lag() {
    ...
    for (name, bars) in [
        ("discount_curve", risk.discount_log_df_adjoints()),
...
fn lsv(t: &HullWhiteLsvTarget, rough: bool, trace: bool, workers: u32) -> Plan {
    ...
    if rough {
        Plan::compile_rough_lsv(
            &req,
            t,
            RoughBergomi::new(0.1, 0.7, -0.5).unwrap(),
            rates(0.01),
            corr,
            particles,
            policy(workers),
...
fn escrowed_lsv_and_rough_lsv_vegakt_reverse_every_recalibrated_quote() {
    ...
    for rough in [false, true] {
        ...
        for (i, &bar) in bars.iter().enumerate() {
            ...
            let b = lsv(&target(down), rough, false, 1)
                .evaluate()
                .unwrap()
                .value;
            close(bar, (a - b) / (2.0 * h));
        ...
        assert_eq!(
            risk.derivatives,
            lsv(&t, rough, true, 3).evaluate_aad().unwrap().derivatives
        );
    }
}
