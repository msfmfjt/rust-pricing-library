...
mod tests {
    ...
    fn zero_vol_of_vol_still_requires_the_realized_calibration_trace() {
        let target = LocalVarianceGrid::new(
            vec![0.0, 0.5, 1.0], vec![-0.8, 0.0, 0.8], vec![0.04; 9], 1e-8, 4.0,
        ).unwrap();
        ...
        assert_eq!(c.validate_calibration_reverse(), Err(LsvError::ReverseTraceNotRetained));
        assert_eq!(c.calibration_pullback(&[0.0; 9]), Err(LsvError::ReverseTraceNotRetained));
        // Direct public reverse keeps its historical seed-validation order.
        assert_ne!(c.reverse_leverage(&[]).unwrap_err(), LsvError::ReverseTraceNotRetained);
    }

    #[test]
    fn hw_preflight_rejects_a_trace_whose_public_primal_changed() {
        let target = HullWhiteLsvTarget::flat(
            0.2, vec![0.0, 0.5, 1.0], vec![-0.8, 0.0, 0.8], 1e-8, 4.0,
        ).unwrap();
        ...
        assert_eq!(c.reverse_leverage(&[0.0; 9]).unwrap_err().to_string(), preflight);
    }
}
