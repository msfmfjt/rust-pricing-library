...
impl sealed::Sealed for LogCoordinate {
    ...
    fn pullback(
        ...
        FactorAdjoints {
            state: LogState { value: next.value * t.decay + vbar * 2.0 * variance },
            spot: next.value * if external { 0.0 } else { t.spot },
            volatility: [next.value * if external { self.vol_of_vol() } else { t.orthogonal }],
        }
    }
}
...
impl BergomiDynamics for LogCoordinate {
    ...
    fn vol_of_vol(self) -> f64 { self.0.vol_of_vol() }
    fn parameters(self) -> Vec<f64> {
        // Distinguish this test representation in the existing plan fingerprint.
        vec![self.0.mean_reversion(), self.vol_of_vol(), self.0.correlation(), 601.0]
    }
    ...
    fn multiplier(self, x: LogState) -> f64 { x.value.exp() }
    fn multiplier_squared(self, x: LogState) -> f64 { (2.0 * x.value).exp() }
    fn finite(x: LogState) -> bool { x.value.is_finite() }
    ...
    fn reverse_factor(
        self, t: LogTransition, next: LogState, vbar: f64, variance: f64, external: bool,
    ) -> (LogState, f64, [f64; 2]) {
        let a = self.pullback(t, next, vbar, variance, InnovationSource::from_external(external));
...
fn request(qmc: bool, bump: f64) -> PricingRequest {
    let mut v: Value = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"), "/../../fixtures/v1/pricing_request.golden.json"
    ))).unwrap();
...
fn alternate_state_uses_shared_calibration_payoff_and_recalibrated_reverse() {
    ...
    for qmc in [false, true] {
        for workers in [1, 2] {
            ...
            let original = BergomiLsvPricingPlan::compile(&req, factor, particles(), policy).unwrap();
            let alternate = BergomiLsvPricingPlan::compile(&req, LogCoordinate(factor), particles(), policy).unwrap();
            assert_ne!(original.plan_fingerprint(), alternate.plan_fingerprint());
            for (&a, &b) in alternate.calibration().surface().squared_leverage().iter()
            ...
            let value = |bump| BergomiLsvPricingPlan::compile(
                &request(qmc, bump), LogCoordinate(factor), particles(), policy,
            ).unwrap().evaluate().unwrap().value;
            close(risk.node_adjoints[4], (value(1e-6) - value(-1e-6)) / 2e-6, 2e-6);
        }
    }
}
...
fn alternate_state_pullback_handles_normals_and_supplied_ou_increments() {
    ...
    for source in [InnovationSource::IndependentNormals, InnovationSource::JointOuIncrements] {
        let loss = |v: &[f64]| {
        ...
        for (i, bar) in [adj.state.value, adj.spot, adj.volatility[0]].into_iter().enumerate() {
            let mut up = input; up[i] += 1e-6;
            let mut down = input; down[i] -= 1e-6;
